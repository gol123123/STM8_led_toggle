  ...
  95                     ; 72 }
  98  000c 81            	ret
 261                     ; 82 void AWU_Init(AWU_Timebase_TypeDef AWU_TimeBase)
 261                     ; 83 {
 262                     	switch	.text
 263  000d               _AWU_Init:
 265  000d 88            	push	a
 266       00000000      OFST:	set	0
 269                     ; 86     assert_param(IS_AWU_TIMEBASE_OK(AWU_TimeBase));
 271  000e 4d            	tnz	a
 ...
 317  0064               L41:
 318                     ; 89     AWU->CSR |= AWU_CSR_AWUEN;
 320  0064 721850f0      	bset	20720,#4
 321                     ; 92     AWU->TBR &= (uint8_t)(~AWU_TBR_AWUTB);
 323  0068 c650f2        	ld	a,20722
 324  006b a4f0          	and	a,#240
 325  006d c750f2        	ld	20722,a
 326                     ; 93     AWU->TBR |= TBR_Array[(uint8_t)AWU_TimeBase];
 ...
 334                     ; 96     AWU->APR &= (uint8_t)(~AWU_APR_APR);
 336  007d c650f1        	ld	a,20721
 337  0080 a4c0          	and	a,#192
 338  0082 c750f1        	ld	20721,a
 339                     ; 97     AWU->APR |= APR_Array[(uint8_t)AWU_TimeBase];
 ...
 350  0092 84            	pop	a
 351  0093 81            	ret
 406                     ; 108 void AWU_Cmd(FunctionalState NewState)
 406                     ; 109 {
 407                     	switch	.text
 408  0094               _AWU_Cmd:
 412                     ; 110     if (NewState != DISABLE)
 414  0094 4d            	tnz	a
 415  0095 2706          	jreq	L531
 416                     ; 113         AWU->CSR |= AWU_CSR_AWUEN;
 418  0097 721850f0      	bset	20720,#4
 420  009b 2004          	jra	L731
 421  009d               L531:
 422                     ; 118         AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 ...
 490                     ; 135 void AWU_LSICalibrationConfig(uint32_t LSIFreqHz)
 490                     ; 136 {
 491                     	switch	.text
 492  00a2               _AWU_LSICalibrationConfig:
 494  00a2 5206          	subw	sp,#6
 495       00000006      OFST:	set	6
 498                     ; 138     uint16_t lsifreqkhz = 0x0;
 500                     ; 139     uint16_t A = 0x0;
 502                     ; 142     assert_param(IS_LSI_FREQUENCY_OK(LSIFreqHz));
 ...
 531  00d5               L03:
 532                     ; 144     lsifreqkhz = (uint16_t)(LSIFreqHz / 1000); /* Converts value in kHz */
 ...
 589  011e 81            	ret
 612                     ; 165 void AWU_IdleModeEnable(void)
 612                     ; 166 {
 613                     	switch	.text
 614  011f               _AWU_IdleModeEnable:
 618                     ; 168     AWU->CSR &= (uint8_t)(~AWU_CSR_AWUEN);
 620  011f 721950f0      	bres	20720,#4
 621                     ; 171     AWU->TBR = (uint8_t)(~AWU_TBR_AWUTB);
 623  0123 35f050f2      	mov	20722,#240
 624                     ; 172 }
 627  0127 81            	ret
 671                     ; 180 FlagStatus AWU_GetFlagStatus(void)
 671                     ; 181 {
 672                     	switch	.text
 673  0128               _AWU_GetFlagStatus:
 677                     ; 182     return((FlagStatus)(((uint8_t)(AWU->CSR & AWU_CSR_AWUF) == (uint8_t)0x00) ? RESET : SET));
 ...
 733                     	switch	.const
 734  002e               L501:
 735  002e 7374645f6c69  	dc.b	"std_lib\src\stm8s_"
 736  0040 6177752e6300  	dc.b	"awu.c",0
 737                     	xref.b	c_lreg
 738                     	xref.b	c_x
 758                     	xref	c_imul
 759                     	xref	c_ludv
