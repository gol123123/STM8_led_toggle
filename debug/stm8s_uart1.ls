 ...
 780  0261 81            	ret
 835                     ; 177 void UART1_Cmd(FunctionalState NewState)
 835                     ; 178 {
 836                     	switch	.text
 837  0262               _UART1_Cmd:
 841                     ; 179     if (NewState != DISABLE)
 843  0262 4d            	tnz	a
 844  0263 2706          	jreq	L732
 845                     ; 182         UART1->CR1 &= (uint8_t)(~UART1_CR1_UARTD); 
 847  0265 721b5234      	bres	21044,#5
 849  0269 2004          	jra	L142
 850  026b               L732:
 851                     ; 187         UART1->CR1 |= UART1_CR1_UARTD;  
 853  026b 721a5234      	bset	21044,#5
 854  026f               L142:
 855                     ; 189 }
 858  026f 81            	ret
 984                     ; 205 void UART1_ITConfig(UART1_IT_TypeDef UART1_IT, FunctionalState NewState)
 984                     ; 206 {
 985                     	switch	.text
 ...
 993                     ; 207     uint8_t uartreg = 0, itpos = 0x00;
 997                     ; 210     assert_param(IS_UART1_CONFIG_IT_OK(UART1_IT));
...
1023  02a3               L46:
1024                     ; 211     assert_param(IS_FUNCTIONALSTATE_OK(NewState));
...
1043  02c0               L27:
1044                     ; 214     uartreg = (uint8_t)((uint16_t)UART1_IT >> 0x08);
1046  02c0 7b03          	ld	a,(OFST+1,sp)
1047  02c2 6b01          	ld	(OFST-1,sp),a
1049                     ; 216     itpos = (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART1_IT & (uint8_t)0x0F));
...
2579  059d               L3701:
2580                     ; 592             status = RESET;
2582  059d 0f01          	clr	(OFST+0,sp)
2584  059f 200f          	jra	L7601
2585  05a1               L1701:
2586                     ; 597         if ((UART1->SR & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
...
2598  05ae               L1011:
2599                     ; 605             status = RESET;
2601  05ae 0f01          	clr	(OFST+0,sp)
2603  05b0               L7601:
2604                     ; 609     return status;
2606  05b0 7b01          	ld	a,(OFST+0,sp)
2609  05b2 5b03          	addw	sp,#3
2610  05b4 81            	ret
2646                     ; 639 void UART1_ClearFlag(UART1_Flag_TypeDef UART1_FLAG)
2646                     ; 640 {
2647                     	switch	.text
2648  05b5               _UART1_ClearFlag:
2650  05b5 89            	pushw	x
2651       00000000      OFST:	set	0
2654                     ; 641     assert_param(IS_UART1_CLEAR_FLAG_OK(UART1_FLAG));
...
2672  05d3               L252:
2673                     ; 644     if (UART1_FLAG == UART1_FLAG_RXNE)
2675  05d3 1e01          	ldw	x,(OFST+1,sp)
2676  05d5 a30020        	cpw	x,#32
2677  05d8 2606          	jrne	L3211
2678                     ; 646         UART1->SR = (uint8_t)~(UART1_SR_RXNE);
2680  05da 35df5230      	mov	21040,#223
2682  05de 2004          	jra	L5211
2683  05e0               L3211:
2684                     ; 651         UART1->CR4 &= (uint8_t)~(UART1_CR4_LBDF);
2686  05e0 72195237      	bres	21047,#4
2687  05e4               L5211:
2688                     ; 653 }
2691  05e4 85            	popw	x
2692  05e5 81            	ret
2775                     ; 668 ITStatus UART1_GetITStatus(UART1_IT_TypeDef UART1_IT)
2775                     ; 669 {
2776                     	switch	.text
...
2784                     ; 670     ITStatus pendingbitstatus = RESET;
2786                     ; 671     uint8_t itpos = 0;
2788                     ; 672     uint8_t itmask1 = 0;
2790                     ; 673     uint8_t itmask2 = 0;
2792                     ; 674     uint8_t enablestatus = 0;
2794                     ; 677     assert_param(IS_UART1_GET_IT_OK(UART1_IT));
...
2822  061e               L262:
2823                     ; 680     itpos = (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART1_IT & (uint8_t)0x0F));
...
2839                     ; 682     itmask1 = (uint8_t)((uint8_t)UART1_IT >> (uint8_t)4);
2841  062f 7b04          	ld	a,(OFST+2,sp)
2842  0631 4e            	swap	a
2843  0632 a40f          	and	a,#15
2844  0634 6b02          	ld	(OFST+0,sp),a
2846                     ; 684     itmask2 = (uint8_t)((uint8_t)1 << itmask1);
...
3083                     	switch	.const
3084  0008               L371:
3085  0008 7374645f6c69  	dc.b	"std_lib\src\stm8s_"
3086  001a 75617274312e  	dc.b	"uart1.c",0
3087                     	xref.b	c_lreg
3088                     	xref.b	c_x
3108                     	xref	c_lsub
3109                     	xref	c_smul
